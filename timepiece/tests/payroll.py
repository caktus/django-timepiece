...
class PayrollTest(TimepieceDataTestCase):
    ...
    def testPersonSummary(self):
        ...
        billable = self.log_time(delta=(3, 30), status='verified')
        non_billable = self.log_time(delta=(2, 0), billable=False, status='verified')
        #summary['total'] does not increase from unverified hours
        unverified = self.log_time(delta=(5, 0), status='unverified')
        sick = self.log_time(delta=(8, 0), project=sick, status='verified')
        vacation = self.log_time(delta=(4, 0), project=vacation, status='verified')
