...
class User(factory.django.DjangoModelFactory):
    ...
    class Meta:
        model = auth.User
        exclude = ('factory_uuid',)

    factory_uuid = str(uuid.uuid4())

    first_name = factory.Sequence(lambda n, factory_uuid=factory_uuid: 'Sam{n}-{uuid}'.format(n=n, uuid=factory_uuid))
    last_name = factory.Sequence(lambda n, factory_uuid=factory_uuid: 'Blue{n}-{uuid}'.format(n=n, uuid=factory_uuid))
    username = factory.Sequence(lambda n, factory_uuid=factory_uuid: 'user{n}-{uuid}'.format(n=n, uuid=factory_uuid))
    email = factory.Sequence(lambda n, factory_uuid=factory_uuid: 'user{n}-{uuid}@example.com'.format(n=n, uuid=factory_uuid))
