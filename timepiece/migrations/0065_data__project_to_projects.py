...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for contract in orm['timepiece.ProjectContract'].objects.all():
            contract.projects.add(contract.project)

            pname = contract.project.name
            bname = contract.project.business.short_name or contract.project.business.name
            if len(pname + bname) > 252:
                contract.name = pname
            else:
                contract.name = '{0} ({1})'.format(pname, bname)
            contract.save()

    def backwards(self, orm):
        for contract in orm['timepiece.ProjectContract'].objects.all():
            contract.project = contract.projects.all()[0]
            contract.save()
