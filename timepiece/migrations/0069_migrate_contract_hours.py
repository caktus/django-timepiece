...
# South doesn't save the class constants from our models, so reproduce the values here
# as of the data schema for this migration.
# Hopefully nobody will be silly enough to change these values later...
PENDING_STATUS = 1
APPROVED_STATUS = 2
...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for contract in orm['timepiece.ProjectContract'].objects.exclude(num_hours=0):
            ...
            ContractHour.objects.create(
                ...
                contract=contract,
                date_requested=contract.start_date,
                date_approved=contract.start_date,
                status=APPROVED_STATUS,
                notes="Hours migrated from contract"
            )
            contract.num_hours = 0
            contract.save()

    def backwards(self, orm):
        """
        Add up the contract hours approved and set the contract hours.
        Abort if there are pending hours; we can't migrate those backwards.
        """
        ContractHour = orm['timepiece.ContractHour']
        ...
        for contract_hour in ContractHour.objects.all():
            contract = contract_hour.contract
            contract.num_hours += contract_hour.hours
            contract.save()

        # Remove the contract hours
        ContractHour.objects.all().delete()
    ...
    models = {
        ...
        'timepiece.contracthour': {
            'Meta': {'object_name': 'ContractHour'},
            'contract': ('django.db.models.fields.related.ForeignKey', [], {'related_name': "'contract_hours'", 'to': "orm['timepiece.ProjectContract']"}),
        ...
        'timepiece.projectcontract': {
            'Meta': {'ordering': "('-end_date',)", 'object_name': 'ProjectContract'},
            'end_date': ('django.db.models.fields.DateField', [], {}),
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'name': ('django.db.models.fields.CharField', [], {'max_length': '255'}),
            'num_hours': ('django.db.models.fields.DecimalField', [], {'default': '0', 'max_digits': '8', 'decimal_places': '2'}),
            'projects': ('django.db.models.fields.related.ManyToManyField', [], {'related_name': "'contracts'", 'symmetrical': 'False', 'to': "orm['timepiece.Project']"}),
            'start_date': ('django.db.models.fields.DateField', [], {}),
