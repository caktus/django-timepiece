...
class Command(BaseCommand):
    ...
    args = _('<user\'s first or last name or user.id> <user\'s first...>...')
    help = _("""Check the database for entries that overlap.
...
./manage.py check_entries --help
    ...
    def make_options(self, *args, **kwargs):
        ...
        make_option('--thisweek',
            ...
            dest='week',
            default=False,
            help=_('Show entries from this week only')),
        ) + (
        make_option('--thismonth',
            action='store_true',
            dest='month',
            default=False,
            help=_('Show entries from this month only')),
        ) + (
        make_option('-y', '--thisyear',
            action='store_true',
            dest='year',
            default=False,
            help=_('Show entries from this year only')),
        ) + (
        make_option('-a', '--all', '--forever',
            action='store_true',
            dest='all',
            default=False,
            help=_('Show entries from all recorded history')),
        ) + (
        make_option('-d', '--days',
            dest='days',
            type='int',
            default=0,
            help=_('Show entries for the last n days only')),
        )
    ...
    def handle(self, *args, **kwargs):
        ...
        if verbosity >= 1:
            print _('Total overlapping entries: %d') % all_overlaps
    ...
    def check_entry(self, entries, *args, **kwargs):
        ...
        if user_total_overlaps and user and verbosity >= 1:
            ...
            print _('Total overlapping entries for user ' + \
    ...
    def find_people(self, *args):
        ...
        if not people.count() and args:
            if len(args) == 1:
                raise CommandError(_('No user was found with the name %s') \
                % args[0])
            else:
                arg_list = ', '.join(args)
                raise CommandError(_('No users found with the names: %s') \
                % arg_list)
        return people
    ...
    def show_init(self, start, *args, **kwargs):
        ...
        if forever:
            if verbosity >= 1:
                print _('Checking overlaps from the beginning ' + \
                    'of time')
        else:
            if verbosity >= 1:
                print _('Checking overlap starting on: ') + \
    ...
    def show_overlap(self, entry_a, entry_b=None, **kwargs):
        ...
        if entry_b:
            ...
            output = _('Entry %(entry)d for %(first_name)s %(last_name)s from ' \
                       '%(start)s to %(end)s on %(project)s overlaps ') % data_a +\
                     _('entry %(entry)d from %(start)s to %(end)s on ' \
                       '%(project)s.') % data_b
        else:
            output = _('Entry %(entry)d for %(first_name)s %(last_name)s from ' \
